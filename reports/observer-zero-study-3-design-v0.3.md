...
| R19 | Families & models | **Revised on a funding constraint, not a scientific one** (2026-08-15): paid OpenAI/Gemini access is unavailable, so the four independent lineages are **claude-sonnet-4-5 (Anthropic)** · **sonar-pro (Meta/Llama + retrieval)** · **r1-1776 (DeepSeek lineage, Perplexity credit — already routed by `factory.ts`, zero build)** · **one of {Gemini Flash free tier, Bedrock Converse: Nova or Mistral}**, with haiku as the within-Claude capability contrast. This spans four architectural traditions rather than three, so the generality claim is not weakened by the substitution; the caveats to state in the paper are that the fourth family is mid-tier rather than flagship, and that r1-1776 is a post-trained derivative of DeepSeek-R1. Exact dated ids and measured prices pinned at the freeze commit after live smoke tests | DECIDE-AT freeze commit (needs the local smoke tests below) |
